...
var Fonts$1 = function Fonts() {
	...
	var opensans = new fontfaceobserver_standalone('Open Sans');
	var playfair = new fontfaceobserver_standalone('Playfair Display');

	Promise.all([
	//		lato.load(),
	opensans.load(), playfair.load()]).then(function () {
...
/*
*	Refactored slightly to work with rollup
*/
...
function googleMapsApiLoader(params) {
  if (googleApi) {
    return Promise$1.resolve(googleApi);
  }
  ...
  return new Promise$1(deferred);
}

var googleMapsApiLoader_1 = googleMapsApiLoader;
...
var GMap = function () {
	...
	createClass(GMap, [{
		...
		value: function initMap() {
			...
			index({
				...
				_this.google = google;
				_this.map = new google.maps.Map(_this.selector, {
					center: {
						lat: 50.8368329,
						lng: -1.0459649
					},
		...
		key: 'createMarker',
		value: function createMarker(markerData, latlng) {
			var html = '<b>' + markerData.name + '</b> <br/> ' + markerData.address;
   ...
   */
		...
		value: function displayMarkers() {
			...
			var latlng = new this.google.maps.LatLng(this.marker.map.Latitude, this.marker.map.Longitude);
...
var Brief = function () {
	...
	createClass(Brief, [{
		...
		value: function initValidation() {
			...
			Validation.ui.config = ValidationConfig$1;
		}
	}, {
		key: 'setFormHeight',
		...
		value: function init() {
			...
			this.actionNext.forEach(function (next) {
				return _this.disableButton(next);
			});
		}
	}, {
		key: 'bindEvents',
		value: function bindEvents() {
			...
			this.actionNext.forEach(function (next) {
				next.addEventListener('click', async function () {
					console.log('closest', next.closest('.js-brief-slide'));
					var isValid = await this.validateSlide(next.closest('.js-brief-slide'));
			...
			this.actionPrev.forEach(function (prev) {
				prev.addEventListener('click', function () {
					_this2.setActiveSlide(_this2.getTargetById(prev));
				});
			});
		}
	}, {
		key: 'getTargetById',
		value: function getTargetById(button) {
		...
		value: function validateSlide(slide) {
			...
			Validation.validateSection(slide).then(function (res) {
				if (res === true) {
					_this3.enableButton(slide.querySelector('.js-brief-next'));
					return res;
				} else {
